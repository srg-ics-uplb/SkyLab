...
class VinaBasicForm(forms.Form):
    ...
    param_flex = forms.FileField(validators=[pdbqt_file_extension_validator], required=False, label="Flex file",
                                 help_text="Flexible side chains, if any (.pdbqt)")
    param_ligands = MultiFileField(min_num=1, validators=[multi_pdbqt_file_validator], label="Ligand file(s)",
                                   help_text="(.pdbqt)")
